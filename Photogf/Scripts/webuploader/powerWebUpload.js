...
(function ($, window) {
    ...
    function initWebUpload(item, options) {
        ...
        var webuploaderoptions = $.extend({
            ...
            // 文件接收服务端。
            server: applicationPath + '/ajax/Process',
        ...
        uploader.on('uploadSuccess', function (file, response) {
            ...
            filePathList += fileEvent.filePath+"|";
        ...
        $list.on("click", ".del", function () {
            ...
            $.each(jsonData.fileList, function (index, item) {
                if (item && item.queueId === id) {
                    ...
                    $.post(applicationPath + '/ajax/Delete', { 'filePathName': deletefile.filePath }, function (returndata) {
                        $ele.parent().remove();
                    });
                    return;
                }
        ...
        $list.on("click", ".download", function () {
            ...
            $.each(jsonData.fileList, function (index, item) {
                if (item && item.queueId === id) {
                    downloadfile = item;
                    $("#" + opts.hiddenInputId).val(JSON.stringify(jsonData));
                    $.post(applicationPath + '/ajax/GetPathId', { 'filePathName': downloadfile.filePath, 'fileName': downloadfile.name }, function (pathid) {
                        window.location.href = applicationPath + '/ajax/Download?pathid=' + pathid;
                    });
                    return;
                }
            });
    ...
    initWebUpload.prototype.deleteAll = function () {
        ...
        jsonData = JSON.parse(fileDataStr);
        $.post(applicationPath + '/ajax/DeleteAll', { 'filelist': jsonData }, function (returndata) {
            self.$list.empty();
        });
    }
